...
$(".modifyAppInfo").on("click",function(){
	...
	var status = obj.attr("status");
	if(status == "1" || status == "3"){//待审核、审核未通过状态下才可以进行修改操作
		window.location.href="getByAppInfoId?id="+ obj.attr("appinfoid");
	}else{
...
$(function(){
	...
	var cataLog2=$("#cataLog2").val();
	var cataLog3=$("#cataLog3").val();
	if(cataLog2!=null&&cataLog2!=""){
		$.ajax({
			type:"POST",//请求类型
			url:"categorylevellist.json",//请求的url
			data:{pid:cataLog1},//请求参数
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				$("#queryCategoryLevel2").html("");
				var options = "<option value=\"\">--请选择--</option>";
				for(var i = 0; i < data.length; i++){
					if(cataLog2==data[i].id){
	...
	if(cataLog3!=null&&cataLog3!=""){
		$.ajax({
			type:"POST",//请求类型
			url:"categorylevellist.json",//请求的url
			data:{pid:cataLog2},//请求参数
			dataType:"json",//ajax接口（请求url）返回的数据类型
			success:function(data){//data：返回数据（json对象）
				$("#queryCategoryLevel3").html("");
				var options = "<option value=\"\">--请选择--</option>";
				for(var i = 0; i < data.length; i++){
					if(cataLog3==data[i].id){
						options += "<option selected='selected'  value=\""+data[i].id+"\">"+data[i].categoryName+"</option>";
					}else{
						options += "<option  value=\""+data[i].id+"\">"+data[i].categoryName+"</option>";
					}
